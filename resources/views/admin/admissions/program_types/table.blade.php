...
@foreach ($programtypes as $key => $programtype)
    <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ $programtype->title }}</td>
        
        <td>
            <div class="edit-1 d-flex align-items-center justify-content-center">
                <a title="Edit Customer" href="{{ route('programTypes.edit', $programtype->id) }}">
                    <span class="edit-icon"><i class="ph ph-pencil-simple"></i></span>
                </a>
                <a title="Delete Customer" data-route="{{ route('programTypes.delete', $programtype->id) }}"
